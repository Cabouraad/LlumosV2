...
BEGIN
  ...
  -- Aggregated stats over the last 30 days (no row payloads)
  agg30 AS (
    SELECT
  ...
  -- Slim recent responses list (no heavy JSON cols)
  recent_responses AS (
    SELECT
  ...
  -- Pre-aggregated 7-day brand & competitor presence (one row per day)
  presence_rows AS (
    SELECT
      DATE(run_at) AS day,
      COUNT(*) AS total,
      COUNT(*) FILTER (WHERE org_brand_present) AS present,
      jsonb_object_agg(comp_lower, hits) FILTER (WHERE comp_lower IS NOT NULL) AS competitor_presence
    FROM (
      SELECT
        a.run_at,
        a.org_brand_present,
      ...
      GROUP BY a.run_at, a.org_brand_present, a.id, LOWER(TRIM(c.name))
    ) x
    GROUP BY DATE(run_at)
  ),
  metrics_7d AS (
  ...
  SELECT jsonb_build_object(
    ...
    'presenceDaily', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'total', pr.total,
        'present', pr.present,
        'competitorPresence', COALESCE(pr.competitor_presence, '{}'::jsonb)
      ) ORDER BY pr.day)
