-- Fix get_org_competitor_summary_v2 to enforce strict brand isolation
-- When p_brand_id is specified, ONLY return competitors with that exact brand_id
-- Do NOT include NULL brand_id records (legacy data) to maintain brand separation
...
CREATE OR REPLACE FUNCTION public.get_org_competitor_summary_v2(
  ...
  p_providers text[] DEFAULT NULL::text[],
...
BEGIN
  ...
  -- STRICT BRAND FILTERING: When p_brand_id is specified, only match competitors 
  -- that have EXACTLY that brand_id. Do NOT include NULL brand_id records.
  valid_expanded AS (
    SELECT e.*
    FROM expanded e
    WHERE e.competitor_name_lower IN (
      ...
      WHERE bc.org_id = v_org_id 
        ...
        AND (
          (p_brand_id IS NULL) OR 
          (bc.brand_id = p_brand_id)  -- Strict: only exact match, no NULL fallback
        )
      UNION
    ...
    COALESCE(
      (
        ...
        WHERE bc.org_id = v_org_id 
          ...
          AND (
            (p_brand_id IS NULL) OR 
            (bc.brand_id = p_brand_id)  -- Strict: only exact match
          )
        LIMIT 1
      ),
