...
BEGIN
  ...
  agg30 AS (
    SELECT
      r.id, r.run_at, r.score, r.org_brand_present, r.competitors_json, r.provider
  ...
  -- 7-day per-provider rollup for "Visibility by AI Platform" chart
  provider_daily AS (
    SELECT
      DATE(run_at) AS day,
    ...
    GROUP BY DATE(run_at), LOWER(COALESCE(provider, ''))
  ),
  metrics_7d AS (
    SELECT
  ...
  SELECT jsonb_build_object(
    ...
    'providerDaily', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'date', pd2.day,
        'provider', pd2.provider,
        'total', pd2.total,
        'present', pd2.present
      ) ORDER BY pd2.day, pd2.provider)
      FROM provider_daily pd2
    ), '[]'::jsonb),
    'metrics', jsonb_build_object(
