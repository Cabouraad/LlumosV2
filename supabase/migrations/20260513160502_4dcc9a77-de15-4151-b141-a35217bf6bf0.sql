...
BEGIN
  ...
  -- Delete dependent data. Order matters for tables that reference prompts.
  ...
  DELETE FROM public.ai_sources_top_domains WHERE brand_id = p_brand_id;
...
REVOKE ALL ON FUNCTION public.delete_brand_cascade(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.delete_brand_cascade(uuid) TO authenticated;