 ...
 *   • Competitive Share of Voice (0–10) — pre-computed SoV ratio (brand vs. competitor recommendation events) × 10
 *
 * Guardrails (must hold for prospect-facing trust):
 *   1. Zero verified mentions  → final = 0 AND every component = 0
 *   2. Single weak mention on a large run (≥20 valid responses) → quality cap keeps final < 15
 *   3. Category Difficulty is NEVER added (caller passes it as diagnostic only)
 *   4. SoV ratio MUST come from `computeShareOfVoice` (which already excludes research-backed entities
 *      that never appeared in AI text). This function does no SoV math itself.
 *   5. Positive/brand-adjacent context handling lives in per-response scoring (`r.score`) which
 *      feeds Mention Quality. This function does no text analysis.
 */
...
export function computeVisibilityScore(
  ...
  // Rationale: with 20+ valid responses, one verified mention — even strong — represents
  // very narrow visibility. Cap final < 15 unless the brand also won on prompt + provider
  // diversity (which it cannot with only one mention).
  ...
  if (verifiedBrandMentions === 1 && validResults.length >= 20) {
    ...
    // Quality cap: even a "perfect" single mention contributes at most 5 quality points
    mentionQualityScore = Math.min(mentionQualityScore, 5);
    // SoV cap: a single recommendation event in a 20+ pool cannot dominate the conversation
  ...
  // Belt-and-suspenders cap for the single-mention case so total stays under 15
  if (singleMentionGuardrailApplied && finalScore >= 15) {
    const overflow = finalScore - 14;
    // Trim from the largest component (mention coverage tends to win in low-N cases)
